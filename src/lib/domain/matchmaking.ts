...
function calculateMatchCost(
  ...
  if (isDoubles) {
    // Coût Partenariats (Poids 1000) - On veut éviter de jouer avec le même partenaire
    ...
    // Coût Oppositions (Poids 1) - On veut varier les adversaires
...
export function generateFullSessionMatches(
  ...
  for (let round = 0; round < roundsCount; round++) {
    ...
    const roundMatches = generateOptimalRound(playersInRound, courts, stats, iterations);
    
    // 3. Appliquer le meilleur round et mettre à jour les stats pour le round suivant
